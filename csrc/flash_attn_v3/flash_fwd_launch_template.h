...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    using SchedulerPersistent = std::conditional_t<Varlen,
        ...
        std::conditional_t<!Is_causal && !Is_local,
            ...
            flash::DynamicPersistentTileScheduler<CollectiveMainloop::NumMmaThreads, NumProducerThreads, Split, PackGQA, Arch >= 90 /*WarpSpecialized*/, Is_flashmask /*Is_flashmask*/>
        >
    >;
                        ...
                        APPENDKV_SWITCH(params.knew_ptr, AppendKV, [&] {
                            ...
                            CLUSTER_SWITCH(cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0, Use_cluster, [&] {
                                static constexpr int ClusterM = Enable_cluster && Use_cluster ? 2 : 1;
                                    run_flash_fwd<Arch, kHeadDim, kHeadDimV, ClusterM, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV && Varlen, HasQv, PackGQA, Split, V_colmajor, Is_flashmask && !Is_FP8>(params, stream);
                            });
                        });
                    });
                });
