...
struct CollectiveMainloopFwdSm90 {
    ...
    load_max_min(Params const& params,
        ...
        const int offset = (bidb * params.h_flashmask + bidh / params.h_h_flashmask_ratio) * nblock_seqlen +
            std::max((nblock_seqlen - (reverse_chunk_idx + 1) * Flashmask_n_block_buffer_length), 0);

        const int thread_idx = threadIdx.x - 32;
        const int length  = Flashmask_n_block_buffer_length < nblock_seqlen ? Flashmask_n_block_buffer_length : nblock_seqlen;
    ...
    generate_n_block(Params const& params,
      ...
      const int32_t base_offset = std::max(nblock_seqlen - (reverse_chunk_idx + 1) * Flashmask_n_block_buffer_length, 0);
      ...
      for(int32_t idx = Flashmask_n_block_buffer_length - 1 - thread_idx % ProducerThreadNum; 
          idx >= (0 - (ProducerThreadNum - Flashmask_n_block_buffer_length % ProducerThreadNum)); idx -= ProducerThreadNum
      ) {
      ...
      if (valid_n_block_num < Flashmask_n_block_buffer_length)
    ...
    load(Params const& params,
        ...
        auto load_flashmask = [&] (auto const& smem_pipe_write) {
            if constexpr (Is_flashmask) {
                ...
                if(n_block_idx < Flashmask_n_block_buffer_length && mask_encode_n_block_smem_[n_block_idx] >= 0) {
        ...
        auto n_block_getter = [&mask_encode_n_block_smem_, &extra_flags_smem](int32_t index) {
            ...
            if (index < Flashmask_n_block_buffer_length) {
